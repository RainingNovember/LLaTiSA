# render_dataset_add_images_concat_prompt.py
# -*- coding: utf-8 -*-
"""
Input:  JSON (NOT jsonl), top-level is a list of samples.
Each sample schema (current):
  - id
  - timeseries (can be [T], [[T]], [[...],[...]] or split keys timeseries_0..N)
  - 2img_prompt
  - prompt
  - option
  - answer (kept as-is)
Output:
  - generate two images per sample:
      plot_{key}.png  (trend plot)
      num_{key}.png   (high-density numeric grid, fidelity-preserving)
  - add field: images = [plot_url, num_url]
  - overwrite field: prompt = 2img_prompt + prompt + option (in that order)
  - keep answer unchanged

Also supports:
  - optional sampling before processing (e.g. half dataset)
  - optional renumber ids after sampling
  - optional filename key to avoid overwrite if ids duplicate
"""
...
def to_matrix(timeseries):
    """
    Convert item['timeseries'] to (T, D) float64 matrix.
    Supports:
      - [T] -> (T,1)
      - [[T]] -> (T,1)
      - [[T],[T],...] -> (T,D) with heuristic transpose if it looks like (D,T)
      - (T,D) already
    """
    ts = timeseries
    ...
    if arr.ndim == 2:
        # Heuristic: (D,T) when few rows and many columns
...
def extract_ts_matrix_and_names(item, fallback_idx=0):
    ...
    Supports:
      1) 'timeseries'
      2) 'timeseries_0', 'timeseries_1', ... (unknown count)
...
# Prompt concat
# -----------------------------
def build_final_prompt(item: dict) -> str:
    """
    New prompt = 2img_prompt + prompt + option (in order).
    """
    p0 = item.get("2img_prompt", "") or ""
    p1 = item.get("prompt", "") or ""
    p2 = item.get("option", "") or ""

    # Keep exact ordering; do not force extra newlines (preserve author formatting)
    return p0 + p1 + p2
...
def fmt_fidelity(v: float) -> str:
    # Fidelity-preserving numeric representation (no 3-decimal rounding)
...
def render_numeric_table(data: np.ndarray, save_path: str, col_names: list,
    ...
    """
    High-density numeric OCR image, fidelity-preserving.
    """
    try:
        T, D = data.shape
...
def process_one(args):
    ...
    try:
        ...
        new_item = dict(item)
        new_item["prompt"] = build_final_prompt(new_item)
        return idx, new_item

    # 2) filenames (avoid overwrite)
...
def main():
    ...
    ap.add_argument("--output", required=True, help="Output JSON with images and updated prompt.")
    ...
    # renumber ids (for sampled subset)
    ...
    tasks = [
        (item, i,
         ...
         args.dpi, args.max_rows_per_col,
    ...
    final = [out[i] if out[i] is not None else data[i] for i in range(len(data))]
