...
def build_count_local_detail(local_events):
    if not local_events:
        return "This time series has no significant local events."
    counts = {}
    for ev in local_events:
        counts[ev["type"]] = counts.get(ev["type"], 0) + 1
    detail_parts = [f"{num} events of type '{etype}'" for etype, num in counts.items()]
    return ", ".join(detail_parts)

def remap_to_annotation(attribute_pool, y):
    """
    将 attribute_pool 转换为结构化的 annotation_mapped 信息。
    保留趋势、季节性、局部事件、数值统计等，同时补充全面的统计字段。
    """
    ann = {
        "trend": None,
        "seasonal": None,
        ...
        "global_stats": None,
        "variability": None,
        "trend_segments_detailed": [],
        "cycle_stats": None
    }

    # === 趋势（全局） ===
    ann["trend"] = determine_trend_from_series(y)

    # === 季节性 ===
    s_type = attribute_pool["seasonal"]["type"].strip().lower()
    ann["seasonal"] = "other"
    # 这里假定 ANNOTATION_MAP 在你的文件里已定义或导入
    for k, v in ANNOTATION_MAP.get("seasonal", {}).items():
        if s_type in [vv.lower() for vv in v]:
            ann["seasonal"] = k
            break
    ann["seasonal_period"] = attribute_pool.get("frequency", {}).get("period", None) \
        if ann["seasonal"] in ("sin", "triangle", "square") else None

    # === 局部事件 ===
    local_events = attribute_pool.get("local", []) or []
    ann["local"] = [ev["type"] for ev in local_events] if local_events else []
    ...
    ann["numerical"] = {
        ...
        "count_local": {
            ...
            "detail": build_count_local_detail(local_events)
        }
    }
    ...
    ann["turning_points"] = {"count": len(turning_points), "points": turning_points}

    # === 全局统计信息 ===
    ann["global_stats"] = {
        "mean": round(float(np.mean(y)), 4),
        "std": round(float(np.std(y)), 4),
        "max": ann["numerical"]["max_point"]["value"],
        "min": ann["numerical"]["min_point"]["value"]
    }

    # === 波动性分析 ===
    mean_val = np.mean(y)
    cv_raw = round(float(np.std(y) / mean_val), 4) if mean_val != 0 else None
    cv_abs = abs(cv_raw) if cv_raw is not None else None
    if cv_abs is None:
        vol_level = None
    elif cv_abs > 1.5:
        vol_level = "high"
    ...
    ann["variability"] = {
        "cv": cv_raw,
        "above_mean_count": int(np.sum(y > mean_val)),
        "below_mean_count": int(np.sum(y < mean_val)),
        "volatility_level": vol_level
    }

    # === 趋势分段详情（如果有） ===
    ann["trend_segments_detailed"] = []
    for seg in attribute_pool.get("trend", {}).get("segments", []):
        seg_stats = y[seg["start"]:seg["end"] + 1]
        ann["trend_segments_detailed"].append({
            "type": seg["type"],
            "start_idx": seg["start"],
            "end_idx": seg["end"],
            "start_value": float(y[seg["start"]]),
            "end_value": float(y[seg["end"]]),
            "mean_value": float(np.mean(seg_stats)),
            "max_value": float(np.max(seg_stats)),
            "min_value": float(np.min(seg_stats)),
            "slope": float((y[seg["end"]] - y[seg["start"]]) / max(1, (seg["end"] - seg["start"])))
        })

    # === 趋势分段摘要 ===
    ann["trend_phases_summary"] = [
        {
            "type": seg["type"],
            "range": (seg["start_idx"], seg["end_idx"]),
            "start_value": seg["start_value"],
            "end_value": seg["end_value"]
        }
        for seg in ann["trend_segments_detailed"]
    ]

    # === 扩展周期统计 ===
    if ann["seasonal"] != "none" and attribute_pool.get("frequency", {}).get("period"):
        period_len = attribute_pool["frequency"]["period"]
        peaks = y[::max(1, int(period_len // 2))]
        valleys = y[::max(1, int(period_len))]
        ann["cycle_stats"] = {
            "cycle_count": round(len(y) / period_len, 1),
            "peak_range": [float(np.min(peaks)), float(np.max(peaks))] if len(peaks) > 0 else None,
            "valley_range": [float(np.min(valleys)), float(np.max(valleys))] if len(valleys) > 0 else None,
            "mean_amplitude": float(np.mean(peaks) - np.mean(valleys)) if len(peaks) > 0 and len(valleys) > 0 else None,
            "period_stability": None  # placeholder，如有周期检测算法可替换
        }
    else:
        ann["cycle_stats"] = None

    # === 关键点列表 ===
    ann["key_points"] = [
        {"label": "first_peak", "pos": int(np.argmax(y)), "value": float(np.max(y))},
        {"label": "first_valley", "pos": int(np.argmin(y)), "value": float(np.min(y))}
    ]

    # === 基础信息 ===
    ann["seq_len"] = len(y)
    ann["start_value"] = float(y[0])
    ann["end_value"] = float(y[-1])
