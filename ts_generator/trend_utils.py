...
def generate_trend_list(points, seq_len):
    ...
    for i in range(len(points) - 1):
        ...
        # 对当前段的点做趋势判定
        segment_x = np.arange(points[i][0], points[i+1][0] + 1)
        segment_y = np.linspace(y_left, y_right, len(segment_x))

        # 均值差法（两端均值差与段内振幅比例）
        seg_total_range = max(segment_y) - min(segment_y)
        if seg_total_range < 1e-8:
            trend = "steady"
        else:
            mid_idx = len(segment_y) // 2
            mean_diff = np.mean(segment_y[mid_idx:]) - np.mean(segment_y[:mid_idx])
            mean_diff_ratio = mean_diff / seg_total_range
            
            # 回归斜率法
            slope, _ = np.polyfit(segment_x, segment_y, 1)
            slope_ratio = slope * len(segment_x) / seg_total_range
            
            if abs(mean_diff_ratio) >= 0.1 and abs(slope_ratio) >= 0.02:
                trend = "increase" if mean_diff_ratio > 0 else "decrease"
            else:
                trend = "steady"
